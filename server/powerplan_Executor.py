...
def run_innovus(tcl_path: pathlib.Path, impl_dir: pathlib.Path, restore_enc: str = "", top_module: str = ""):
    ...
    # Build Innovus command - for powerplan, we use the TCL script directly
...
def main():
    parser = argparse.ArgumentParser(description="Powerplan Executor - Execute powerplan TCL scripts")
    ...
    logging.info(f"Starting powerplan execution for design: {args.design}")
    ...
    if args.force:
        outputs_to_clean = [
            impl_dir / "pnr_save" / "powerplan.enc",
            impl_dir / "pnr_reports" / "powerplan.rpt"
        ]
    ...
    if not success:
        logging.error("Powerplan execution failed")
        sys.exit(1)
    ...
    logging.info("Powerplan execution completed successfully")
    sys.exit(0)
