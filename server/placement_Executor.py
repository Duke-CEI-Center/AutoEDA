...
def run_placement_from_tcl(tcl_file: pathlib.Path, workspace_dir: pathlib.Path, force: bool = False):
    """Execute placement using the generated TCL file"""
    
    print(f"Starting placement execution...")
    ...
    try:
        ...
        if not done_file.exists():
            raise RuntimeError("Placement did not complete successfully (_Done_ file not found)")
        
        # Check for placement.enc.dat
        placement_enc = workspace_dir / "pnr_save" / "placement.enc.dat"
        if placement_enc.exists():
            print(f"✓ Placement design saved: {placement_enc}")
        else:
            placement_enc_file = workspace_dir / "pnr_save" / "placement.enc"
            if placement_enc_file.exists():
                print(f"✓ Placement design saved: {placement_enc_file}")
            else:
                print("⚠ Warning: placement.enc(.dat) not found")
...
def main():
    parser = argparse.ArgumentParser(description="MCP EDA Placement Executor")
    parser.add_argument("-mode", required=True, help="Execution mode (placement)")
    ...
    print("=" * 60)
    print("MCP EDA Placement Executor")
    print("=" * 60)
    ...
    try:
        if args.mode == "placement":
            ...
            run_placement_from_tcl(tcl_file, workspace_dir, args.force)
        else:
