...
def run_innovus(tcl_path: pathlib.Path, impl_dir: pathlib.Path, restore_enc: str = "", top_module: str = ""):
    ...
    # Build Innovus command - for CTS, we use the TCL script directly
...
def main():
    parser = argparse.ArgumentParser(description="CTS Executor - Execute CTS TCL scripts")
    ...
    logging.info(f"Starting CTS execution for design: {args.design}")
    ...
    if args.force:
        outputs_to_clean = [
            impl_dir / "pnr_save" / "cts.enc",
            impl_dir / "pnr_reports" / "cts_summary.rpt",
            impl_dir / "pnr_reports" / "postcts_opt_max_density.rpt",
            impl_dir / "pnr_reports" / "ccopt.txt"
        ]
    ...
    if not success:
        logging.error("CTS execution failed")
        sys.exit(1)
    ...
    logging.info("CTS execution completed successfully")
    sys.exit(0)
