...
async def placement(
    ...
    Example:
        placement(design="b14", top_module="b14", tech="FreePDK45", g_idx=0, p_idx=0)
...
# power_planning function removed - now integrated into placement() function

# Legacy placement function removed - now integrated into placement() function
...
async def routing(
    ...
    Returns:
        JSON string containing routing results with status, log path, output files, and archive data
        
    Example:
        routing(design="b14", top_module="b14", restore_enc="/path/to/cts.enc.dat", archive=True)
...
async def complete_eda_flow(
    ...
    try:
        ...
        return json.dumps({
            ...
            "top_module": top_module,
            "tech": tech,
            "message": "Complete EDA flow executed successfully using 4-server architecture",
            "steps_completed": 4,
            "steps": results
        }, ensure_ascii=False, indent=2)
