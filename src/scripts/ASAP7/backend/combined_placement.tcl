...
setDesignMode -flowEffort $FLOW_EFFORT -powerEffort $GLOBAL_POWER_EFFORT -process 7
...
setDrawView fplan
#for asap7
# Unique stdcell sites used by instances in the current design
set sites [dbGet -u top.insts.cell.site.name]
puts "Sites used: $sites"

# Pick the first site (usually there’s only one for stdcells)
set sc_site [lindex $sites 0]
puts "STD site = $sc_site"

# Row height (use get_db on the site object)
set row_h [get_db site:$sc_site .size.y]
puts "Row height = $row_h um"

floorPlan -site $sc_site -r $ASPECT_RATIO $TARGET_UTIL
